...
import FileIcon from "../icons/filebooks.svg";
...
// import XlsxIcon from '../icons/xlsxicon.svg'
...
import { divide, isEmpty, result } from "lodash-es";
...
function imgBg(src: any, txt: string, obj = {}) {
  return <img src={src} alt="" style={obj}></img>
}
...
export function ChatAction(props: {
  ...
  return (
    ...
    >
      <div ref={iconRef} className={styles["icon"]} style={{ "width": `${width.icon}px`, "height": `${width.full}px` }}>
        {props.icon}
      </div>
...
function C_Hat() {
  ...
  function readPdfFile(url: String) {
      ...
      .then(async (pdf) => {
        ...
        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
          ...
          const pageText = text.items.map((item) => {
            const textItem = item as pdfjsLib.TextItem; // 使用 TypeScript 明确声明类型
            return textItem.str;
          }).join(' ');
    ...
    // reader.onload = (e: ProgressEvent<FileReader>) => {
    //   if (e.target?.result && typeof e.target.result !== 'string') {
    //     const typedarray = new Uint8Array(e.target.result as ArrayBuffer); // 类型断言为 ArrayBuffer
    //     getDocument(typedarray).promise.then(async (pdf) => {
    //       // pdf.getPage(1).then((page) => {
    //       //   page.getTextContent().then((textContent) => {
    //       //     console.log('PDF内容:', textContent.items.map((item:any) => item.str).join(' '));
    //       //   });
    //       // });
    //       console.log('PDF loaded');
    //       const textContent = [];
    ...
    <div className={styles.chat} key={session.id}>
                  ...
                  <div className={styles["chat-message-item"]}>
                    ...
                    ========
                      <div >
                        ...
                        { message.fContent[0] && message.fContent[0].indexOf("xlsx") !== -1 && <img src={ "xlsx.png"} alt="" style={{width: "200px", height:"200px"}}></img>}
                        { message.fContent[0] && message.fContent[0].indexOf("docx") !== -1 && <img src={"word.png"} alt="" style={{width: "200px", height:"200px"}}></img>}
                        <div>{message.fContent[0]}</div>
                      </div>
      ...
      <div className={styles["chat-input-panel"]}>
        ...
        <label
          ...
          htmlFor="chat-input"
        >
          {/* {attachFiles.length}{JSON.stringify(attachFiles) } */}
          <textarea
            id="chat-input"
            ref={inputRef}
          ...
          {attachFiles.length != 0 &&(<div className={styles["attach-images"]}>
              {
                <div className={styles["attach-image"]}>
                  ...
                  <div style={{ fontSize: "12px", textOverflow: "ellipsis", overflow: "hidden",whiteSpace:"nowrap" }}>{attachFiles[0]}</div>
          ...
          <IconButton
            ...
            type="primary"
            onClick={() => doSubmit(userInput || fileInput)}
          />
        </label>
      </div>
