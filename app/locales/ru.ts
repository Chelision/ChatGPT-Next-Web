...
const ru: PartialLocaleType = {
  ...
  Error: {
    ...
    \\ 1️⃣ Если вы хотите начать без настройки, [нажмите здесь, чтобы немедленно начать разговор 🚀](${SAASC_Hat_UTM_URL})
 ...
 `,
  ...
  Auth: {
    ...
    TopTips:
      "🥳 Предложение по запуску KnChat AI: разблокируйте OpenAI o1, GPT-4o, Claude-3.5 и новейшие большие модели прямо сейчас",
  },
  ChatItem: {
  ...
  Settings: {
    ...
    Access: {
      SaasStart: {
        Title: "Используйте KnChat AI",
        Label: "(Самое экономичное решение)",
        SubTitle:
          "Официально поддерживается KnChat, готов к использованию без настройки, поддерживает последние крупные модели, такие как OpenAI o1, GPT-4o и Claude-3.5",
        ChatNow: "Начать чат",
      },
